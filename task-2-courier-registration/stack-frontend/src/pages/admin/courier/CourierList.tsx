...
const CourierList = () => {
  ...
  const [courierList, setCourierList] = React.useState<DataType[]>([]);
  const loadCourierList = () => {
    AxiosService()
      .get("/courier/list", { headers: { isShowLoading: true } })
      .then((response: any) => {
        let total = 0;
        const { statusCode, data } = response.data;
        if (parseInt(statusCode) >= 200 && parseInt(statusCode) <= 299) {
          total = parseInt(data.total);
          let courierList: DataType[] = data.couriers;
          let nextState: DataType[] = produce(courierList, (draft) => {
            draft.forEach((item: DataType) => {
              item.key = item.id ? item.id.toString() : "";
              item.name = item.lastname + " " + item.firstname;
            });
          });
          setCourierList(nextState);
        } else {
          setCourierList([]);
        }
        setTableParams({
  ...
  const handleNewForm = () => {
    navigate("/admin/courier/add");
  };
  const handleEdit = (id: number) => () => {
    navigate("/admin/courier/edit/" + id);
  };
  const handleDelete = (id: number) => () => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        AxiosService()
          .delete("/courier/delete/" + id, { headers: { isShowLoading: true } })
          .then((response: any) => {
    ...
    <Card
      title={
        <div className={clsx(["flex", "justify-between"])}>
          <span className={clsx(["text-3xl"])}>{t("Couriers")}</span>
          <AppButton lblCtrl={t("New")} iconCtrl={<PlusOutlined />} onClickForm={handleNewForm} />
        </div>
      }
    >
      <Table<DataType> rowKey="id" columns={columns} dataSource={courierList} pagination={tableParams.pagination} />
    </Card>
  );
};
