...
- Courier Eligibility Constraints: Only couriers with `approvalStatus === APPROVED` and `operationalStatus === ACTIVE` (signifying ONLINE) are queried from the database or filtered in memory to receive delivery notifications.
